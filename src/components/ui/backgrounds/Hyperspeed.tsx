...
// Type for the App class used in useRef
interface AppType {
  dispose: () => void;
}
...
const Hyperspeed = ({ effectOptions, speed, color }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    class CarLights {
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          let carLightColor = pickRandom(colors) as unknown as THREE.Color;

          aColor.push(carLightColor.r);
          aColor.push(carLightColor.g);
          aColor.push(carLightColor.b);
    ...
    class LightsSticks {
      ...
      init() {
        ...
        for (let i = 0; i < totalSticks; i++) {
          ...
          let stickColor = pickRandom(colors) as unknown as THREE.Color;
    ...
    const options = {
      ...
      ...effectOptions,
      speedUp: speed !== undefined ? speed : (effectOptions?.speedUp ?? DEFAULT_EFFECT_OPTIONS.speedUp),
      colors: {
        ...DEFAULT_EFFECT_OPTIONS.colors,
        ...(effectOptions?.colors || {}),
        sticks: color ? parseInt(color.replace('#', '0x'), 16) : (effectOptions?.colors?.sticks ?? DEFAULT_EFFECT_OPTIONS.colors.sticks)
      }
    };
  ...
  return <div ref={hyperspeed} style={{ width: '100%', height: '100%', position: 'absolute', overflow: 'hidden' }}></div>;
};
